...
function updateLetterOverlays(pageIndex) {
  ...
  if (currentIndex > 0) {
    lettersToShow.push({
      letter: alphabet[currentIndex - 1],
  ...
  for (const item of lettersToShow) {
    ...
    const leftPosition =
      leftDividerIndex === 0
        ? 0
        : (letterIndices[alphabet[leftDividerIndex]] / wordList.length) *
          availableWidth;
    const rightPosition =
      rightDividerIndex >= alphabet.length
        ? availableWidth
        : (letterIndices[alphabet[rightDividerIndex]] / wordList.length) *
          availableWidth;
...
function syncSliderWithScroll() {
  ...
  const pageIndex = Math.floor(
    (displayArea.scrollTop + displayArea.offsetHeight / 2) / pageHeight,
  );
  ...
  if (pageIndex >= 0 && pageIndex < displayArea.children.length) {
    ...
    if (!isDragging) {
      ...
      const normalizedPosition =
        pageIndex / (totalPages === 1 ? 1 : totalPages - 1);
...
function calculateLetterFit(dividerPositions) {
  ...
  const letterMarkerStyle = window.getComputedStyle(
    document.querySelector(".letter-marker"),
  );
...
function initializeInterface() {
  ...
  // and scroll page containers.
  displayArea.addEventListener("wheel", (e) => {
    // Get all page containers
    const pageContainers = displayArea.querySelectorAll(".page-container");
